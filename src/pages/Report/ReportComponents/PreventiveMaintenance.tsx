...
import { Badge, Button, DatePicker, Flex, Space, Typography } from "antd";  
import { ColumnsType } from "antd/es/table";  
import * as dayjs from "dayjs";  
import { useMemo, useState, useEffect, JSXElementConstructor, ReactElement, ReactNode, ReactPortal } from "react";  
  ...
  const columns: ColumnsType<IPreventiveMaintenanceReportModel> = useMemo(() => {  
    return [  
      ...
      {  
        ...
        sorter: true,  
        sortDirections: ["descend", "ascend"],  
        render: (status:  "Done" | "Cancel", record: { information: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | null | undefined; }) => (  
          <div>
          <Badge  
  ...
  const handleDownload = async (record: any) => {  
    try {  
      ...
      const transformedData = {  
        ...
        proofOfVisitImages: (() => {  
          const images = record.MediaJobOrderReportProofOfVisit.map((item: any) => ({  
            media_id: item.media_id,  
            media: { path: `http://localhost:46/media/${item.media_id}` }  
          }));  
          console.log("Transformed ProofOfVisit:", images);  
          return images;  
        })(),  
      
        optionalPhotos: (() => {  
          const photos = record.MediaJobOrderReportOptionalPhoto.map((item: any) => ({  
            media_id: item.media_id,  
            media: { path: `http://localhost:46/media/${item.media_id}` }  
          }));  
          console.log("Transformed OptionalPhotos:", photos);  
          return photos;  
        })(),  
